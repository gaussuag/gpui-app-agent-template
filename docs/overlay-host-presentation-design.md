...
日期：2026-09-22。状态：**设计提案，尚未实现；阶段 0 原生验证通过后才能冻结激活细节。**

实施进展：[阶段 0 验证记录](overlay-presentation-stage0.md)。异步宿主层级候选已复现
过期后生效，尚未通过架构门槛；后续实现等待宿主协作/保证范围的选择。

当前代码基线：`f1d4bb55fffd368800766a8876f858f27ea72ef5`。
本方案承接 [Overlay spec](overlay-gpui-spec.md)，讨论用户新要求：不建立原生 owner/parent，
Overlay 在视觉、遮挡和交互上接近宿主内嵌内容。此次只提交方案，不修改运行时。
...
推荐 A；将 C 保留为明确的决策分支，不把它暗中混入本期支持范围。
...
必须验证宿主暂停消息泵→用户切到第三窗口→宿主恢复后，没有迟到的宿主前置。
若不能满足，A 不通过；回到 C 的宿主线程协作方案，或明确讨论降低保证，不能自动放宽验收。
...
| 宿主挂起 | Overlay/UI 关闭仍响应；线程/队列不增长；宿主恢复不得执行过期前置 |
...
设计不新增常驻激活 worker。已投递给外部线程的请求无法由这个关闭序列撤回，仍受阶段 0 阻断条件约束。
...
3. 暂停宿主 UI 线程，发出真实点击，再切第三窗口/关闭 Overlay，恢复宿主：无迟到前置，无 UI 卡住。
...
阶段 0 完成后记录确定的原语、flags、时序、失败语义；此后才开展后续阶段。
通过有限次暂停/恢复实验，不等于证明异步请求不会迟到。候选还必须依据原语契约或执行前校验机制，
说明如何阻止过期效果；实测仅补充证据。裸 SWP_ASYNCWINDOWPOS 没有撤销或执行前校验能力，
不能因为测试暂未复现迟到而获准满足该保证。找不到这样的机制就判定该候选不通过。
...
**首要架构阻断项是外部宿主前置原语能否同时满足第二种交互目标、无 UI 等待和无过期晚到效果。**
这必须由阶段 0 决定。若失败，方案 C 需单独确认接入范围；本次不把原型候选写成已证实能力。
模态关系、topmost 分组切换、桌面切换及原生子控件也必须通过阶段 0 的对应验证。
跨窗口原子性、独占全屏、特殊游戏反作弊/渲染行为不在本次保证内；普通窗口和无边框窗口仍是目标。

本次工作为固定提交/当前源码分析、官方契约核对、三种 Interface 比较和文档设计。
没有运行 C++ SDK 构建、没有修改参考 SDK、没有执行新层级原型，也没有改变当前 Demo 行为。
